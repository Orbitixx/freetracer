...
fn readPrimaryVolumeDescriptor(bytes: [ISO_SECTOR_SIZE]u8) !iso9660.PrimaryVolumeDescriptor {
    if (bytes[0] != 0x01) return error.InvalidPrimaryVolumeDescriptorFirstByte;

    return iso9660.PrimaryVolumeDescriptor{
        .typeCode = bytes[0],
        .standardIdentifier = bytes[1..6].*,
        .version = bytes[6],
        .unused1 = bytes[7],
        .systemIdentifier = bytes[8..40].*,
        .volumeIdentifier = bytes[40..72].*,
        .unused2 = bytes[72..80].*,
        .volumeSpaceSize = bytes[80..88].*,
        .unused3 = bytes[88..120].*,
        .volumeSetSize = bytes[120..124].*,
        .volumeSequenceNumber = bytes[124..128].*,
        .logicalBlockSize = bytes[128..132].*,
        .pathTableSize = bytes[132..140].*,
        .locationOfTypeLPathTable = bytes[140..144].*,
        .locationOfOptionalTypeLPathTable = bytes[144..148].*,
        .locationOfTypeMPathTable = bytes[148..152].*,
        .locationOfOptionalTypeMPathTable = bytes[152..156].*,
        .rootDirectoryEntry = bytes[156..190].*,
        .volumeSetIdentifier = bytes[190..318].*,
        .publisherIdentifier = bytes[318..446].*,
        .dataPreparerIdentifier = bytes[446..574].*,
        .applicationIdentifier = bytes[574..702].*,
        .copyrightFileIdentifier = bytes[702..739].*,
        .abstractFileIdentifier = bytes[739..776].*,
        .bibliographicFileIdentifier = bytes[776..813].*,
        .volumeCreationDateTime = bytes[813..830].*,
        .volumeModificationDateTime = bytes[830..847].*,
        .volumeExpirationDateTime = bytes[847..864].*,
        .volumeEffectiveDateTime = bytes[864..881].*,
        .fileStructureVersion = bytes[881],
        .unused4 = bytes[882],
        .applicationUsed = bytes[883..1395].*,
        .reserved = bytes[1395..2048].*,
    };
}

pub fn validateISOFileStructure(file: std.fs.File) ISO_PARSER_RESULT {
    ...
    const isoBytesRead = file.read(&isoBuffer) catch |err| {
        Debug.log(.ERROR, "Unable to read the specified ISO file. Error: {any}", .{err});
    ...
    for (16..isoSectorCount) |i| {
        ...
        if (sectorBuffer[0] == 0x01) {
            //
            // TODO: implement more precise result items;
            //
            primaryVolumeDescriptor = readPrimaryVolumeDescriptor(sectorBuffer) catch |err| {
                Debug.log(.ERROR, "Unable to read Primary Volume Descriptor. Error: {any}", .{err});
                return ISO_PARSER_RESULT.ISO_INVALID_REQUIRED_VOLUME_DESCRIPTORS;
            };

            if (!std.mem.eql(u8, &primaryVolumeDescriptor.standardIdentifier, "CD001")) {
                Debug.log(.ERROR, "Invalid Primary Volume Descriptor standard identifier. Expected 'CD001', got '{s}'.", .{primaryVolumeDescriptor.standardIdentifier});
                return ISO_PARSER_RESULT.ISO_INVALID_REQUIRED_VOLUME_DESCRIPTORS;
            }
...
pub fn parseIso(allocator: std.mem.Allocator, isoPath: []const u8, deviceSize: i64) ISO_PARSER_RESULT {
    ...
    const file: std.fs.File = std.fs.openFileAbsolute(isoPath, .{ .mode = .read_only, .lock = .exclusive }) catch |err| {
